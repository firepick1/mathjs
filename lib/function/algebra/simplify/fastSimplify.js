...
function factory(type, config, load, typed, math) {
  ...
  /**
   * fastSimplify() performs a very quick optimization pass over
   * an expression tree to reduce computational complexity.
   * For performance, no effort is made to refactor an expression 
   * into a canonical form. 
   *
   * Syntax:
   *
   *     simplify.fastSimplify(expr)
   *
   * Examples:
   *
   *     var f = math.parse('2 * 1 * x ^ (2 - 1)');
   *     math.simplify.fastSimplify(f);                          // Node {2 * x}
   *     math.simplify('2 * 1 * x ^ (2 - 1)', [math.simplify.fastSimplify]); // Node {2 * x};
   *
   * See also:
   *
   *     derivative 
   ...
   */
  function fastSimplify(node) {
    if (node.isOperatorNode && node.args.length <= 2) {
      var a0 = fastSimplify(node.args[0]);
      var a1 = node.args[1] && fastSimplify(node.args[1]);
      if (node.op === "+") {
        ...
        var c = fastSimplify(node.content);
        if (c.isParenthesisNode || c.isSymbolNode || c.isConstantNode) {
            return c;
        }
        return new ParenthesisNode(c);
    } else if (node.isFunctionNode) {
        var args = node.args.map((arg) => fastSimplify(arg));
        if (args.length === 1) {
